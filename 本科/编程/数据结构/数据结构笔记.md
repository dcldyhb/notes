...
hfTree<Type>::hfTree(const Type *v , const int *w , int size)
{
  ...
  for (int i = size ; i < length ; i++) // 对数组 `elem` 中的节点进行初始化，前 $\text{size}$ 个节点存储待编码的元素
  {
...
```
  ...
  - 前 $\text{size}$ 个节点存储待编码的元素
  - 从下标 `size` 开始，依次存储待编码的元素的权值
    - `elem[i].weight = w[i-size]`：将权值存储到节点中
    - `elem[i].data = v[i-size]`：将待编码的元素存储到节点中
...
- `for (int j = i + 1 ; j < length ; j++)``：遍历当前节点之后的所有节点
  ...
  - `if (elem[j].weight < min1)`、`else if(elem[j].weight < min2)`：找到真正的最小值和次小值以及他们对应的下标
- 
