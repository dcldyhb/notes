\documentclass[lang = zh , final , oneside , openany , titlepage , zihao = -4 , linespread = 1.3 , baselineskip = false , cjk-font = windows , text-font = newtx , math-font = newtx , math-style = ISO , uppercase-greek = upright , integral-limits = false]{sjtureport}
...
\begin{theorem}
    设变换 \(\displaystyle T :\begin{aligned}x=x(u,v)\\y=y(u,v)\end{aligned}\) 有连续偏导数，且满足 
...
\item
  在广义极坐标
  \(\begin{aligned}x=ar\cos\theta\\y=br\sin\theta\end{aligned}\)
  下，\(J = abr\)
\end{enumerate}
...
\begin{theorem}
    设变换 \(T:\begin{aligned}x=x(u,v,w)\\y=y(u,v,w)\\z=z(u,v,w)\end{aligned}\)
有连续偏导数，且满足
...
\[\displaystyle \sum_{n=1}^\infty \frac{(-1)^{n}}{n^p}\left\{\begin{aligned}\text{绝对收敛} & ,p>1\\ \text{条件收敛} & , 0 < p\leq 1 \end{aligned}\right.\]
\end{remark}
