...
2. 若幂级数 $\displaystyle \sum_{n=0}^\infty a_nx^n$ 的收敛半径 $R>0$，在收敛区间 $(-R,R)$ 内的和函数为 $S(n)$，则 $S(n)$ 在 $(-R,R)$ 上连续；若 $\displaystyle\sum_{n=0}^\infty a_nx^n$ 在 $x=R$ 或 $x=-R$ 收敛，则和函数 $S(n)$ 在 $x=R$ 左连续或 $x=-R$ 处右连续
