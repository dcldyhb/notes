...
- $\displaystyle\sum_{k=n+1}^\infty a_k$ 称为级数 $\sum _{n=1} a_n$ 的**余项级数**

**定义：**

- 若级数 $\displaystyle\sum_{n=1}^\infty a_n$ 的部分和数列 $\{S_n\}$ 收敛，且 $\displaystyle \lim_{n\to\infty}S_n = S$，则称级数 $\displaystyle \sum_{n=1}^\infty a_n$ **收敛**，$S$ 称为级数 $\displaystyle \sum _{n=1}^\infty a_n$ 的**和**，记作 $\displaystyle \sum_{n=1}^\infty a_n = S$
...
3. **可加性：** 将级数增加、删减或改换**有限项**，不改变级数的敛散性
