<head>
    ...
    MathJax.Hub.Config({
        ...
        TeX: {
            Macros: {
                // 在这里定义你自己的命令
                // 示例：定义一个 \arc 命令，作为 \wideparen 的别名
                // #1 代表第一个参数
                arc: ['\\wideparen{#1}', 1] 
            }
        }
...
- 当曲线 $C$ 为极坐标 $r=r(\theta)$，$\theta\in [\alpha,\beta]$ 时，则 $\displaystyle\int_Cf(x,y)\,\mathrm{d}s = \int_\alpha^\beta f(r(\theta)\cos\theta,r(\theta)\sin\theta)r(\theta)\,\mathrm{d}\theta$
