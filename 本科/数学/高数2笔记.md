...
设 $\Omega$ 是 $\mathbb{R}^3$ 中的有界闭区域，三元函数 $f(x,y,z)$ 在 $\Omega$ 上有界，$I$ 为实数. 若将 $\Omega$ 任意分割成 $n$ 个小区域 $\Delta \Omega_1,\Delta\Omega_2,\ldots,\Delta\Omega_n$，任取 $\left(\varepsilon_i,\eta_i,\xi_i \right)\in\Delta\Omega_i\left(i=1,2,\ldotp,n\right)$，作和 $\displaystyle\sum_{i=1}^nf\left(\varepsilon_i,\eta_i,\xi_i \right)\Delta V_i=I$
