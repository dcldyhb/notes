...
旋转矢量 $\boldsymbol{A}$ 在 $x$ 轴上的投影点的运动规律为
...
$$
\boldsymbol{u}_{x\text{平均速度}} \left(\sum_i^N \frac{u_{ix}}{N}\right) = \bar{u}_{y\text{平均速度}} =\bar{u}_{z\text{平均速度}} =0
$$
...
$$
f(\boldsymbol{v})= \left(\frac{m}{2\pi kT}\right)^{\frac{3}{2}} e^{-\frac{m\boldsymbol{v}^2}{2kT}}
$$
...
\begin{aligned}
  N&=n\bar{v}S\\
  &=nS\int_0^\infty f(v)v\,\mathrm{d}v\\
  &=
\end{aligned}
$$
